...
export default async function DashboardPage() {
  ...
  const [{ data: upcomingDeadlines }, { data: recentSyllabi }, { data: profile }, { data: grades }] =
    await Promise.all([
      supabase.from('deadlines').select('*, course:courses(name, color, code)').eq('user_id', user!.id).eq('completed', false).gte('due_date', today).order('due_date', { ascending: true }).limit(8),
      supabase.from('syllabi').select('*, course:courses(name)').eq('user_id', user!.id).order('created_at', { ascending: false }).limit(4),
  ...
  return (
    <div className="max-w-4xl mx-auto px-8 py-10">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
        <StatCard icon={Clock} label="Upcoming" value={String(upcomingDeadlines?.length ?? 0)} sub="deadlines" color="indigo" />
        <StatCard icon={FileUp} label="Syllabi" value={String(recentSyllabi?.length ?? 0)} sub="uploaded" color="violet" />
        <StatCard icon={TrendingUp} label="GPA" value={cumulativeGpa !== null ? cumulativeGpa.toFixed(2) : '—'} sub={cumulativeGpa !== null ? 'cumulative' : 'add grades'} color="emerald" href="/gpa" />
        <StatCard icon={BookMarked} label="Courses" value={String(recentSyllabi?.length ?? 0)} sub="with syllabi" color="amber" />
      </div>

      <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
        <div className="md:col-span-3 rounded-2xl bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 overflow-hidden shadow-sm">
          <div className="px-5 py-4 border-b border-gray-100 dark:border-zinc-800 flex items-center justify-between">
            ...
            <Link href="/calendar" className="text-xs text-gray-400 dark:text-zinc-500 hover:text-indigo-600 dark:hover:text-zinc-300">View calendar →</Link>
          </div>
          {!upcomingDeadlines?.length ? (
            <EmptyState icon={Clock} message="No upcoming deadlines" sub="Upload a syllabus to get started" />
          ) : (
                ...
                return (
                  <li key={d.id} className="flex items-center gap-3 px-5 py-3 hover:bg-gray-50 dark:hover:bg-zinc-800/30">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <span className={`text-xs font-medium ${urgent ? 'text-red-500' : 'text-gray-400 dark:text-zinc-400'}`}>
        ...
        <div className="md:col-span-2 rounded-2xl bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 overflow-hidden shadow-sm">
          <div className="px-5 py-4 border-b border-gray-100 dark:border-zinc-800 flex items-center justify-between">
            <h2 className="text-sm font-semibold text-gray-900 dark:text-zinc-100">Recent uploads</h2>
            <Link href="/courses" className="text-xs text-gray-400 dark:text-zinc-500 hover:text-indigo-600 dark:hover:text-zinc-300">All courses →</Link>
          </div>
          {!recentSyllabi?.length ? (
            <EmptyState icon={FileUp} message="No uploads yet" sub="Go to Courses to upload a syllabus" />
          ) : (
            <ul className="divide-y divide-gray-100 dark:divide-zinc-800/60">
              {recentSyllabi.map((s) => (
                <li key={s.id} className="px-5 py-3 hover:bg-gray-50 dark:hover:bg-zinc-800/30">
...
async function OverdueAlert(supabase: ReturnType<typeof createClient>, userId: string, today: string) {
  const { data } = await supabase.from('deadlines').select('id').eq('user_id', userId).eq('completed', false).lt('due_date', today).limit(1)
  if (!data?.length) return null
  return (
    <div className="mt-6 flex items-center gap-3 px-4 py-3 rounded-xl bg-red-50 dark:bg-red-500/5 border border-red-200 dark:border-red-500/20 text-sm text-red-600 dark:text-red-400">
...
function StatCard({ icon: Icon, label, value, sub, color, href }: { icon: React.ElementType; label: string; value: string; sub: string; color: string; href?: string }) {
  ...
  const inner = (
    <>
      ...
      <p className="text-xl font-bold tracking-tight text-gray-900 dark:text-zinc-100">{value}</p>
  ...
  if (href) return <Link href={href} className={`${base} hover:border-indigo-300 dark:hover:border-zinc-700 block`}>{inner}</Link>
...
function EmptyState({ icon: Icon, message, sub }: { icon: React.ElementType; message: string; sub: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-10 px-4 text-center">
      <Icon className="w-8 h-8 text-gray-300 dark:text-zinc-700 mb-3" />
